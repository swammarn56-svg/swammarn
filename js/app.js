...
async function saveOperationRow(btn){
...
async function confirmPurchase(id){const p=purchases.find(x=>x.id===id);if(!p||p.confirmed)return;const r=await supabase.from("purchases").update({confirmed:true,applied:true}).eq("id",id);if(r.error){alert(r.error.message);return}const led=await supabase.from("inventory_ledger").insert({event_date:p.purchase_date,item_id:p.item_id,source_type:"purchase",source_id:p.id,qty_delta:p.base_quantity,unit_cost:p.cost_per_base_unit,value_delta:p.total_cost,created_by:await userId()});if(led.error){await supabase.from("purchases").update({confirmed:false,applied:false}).eq("id",id);alert(led.error.message);return}await loadAll()}
